...
static entrance_export[185], exit_export[185], entrance_name[32], last_entrance;
CMD:createentrance(playerid, params[])
{
    ...
    if(sscanf(params, "s[32]", entrance_name))                      return SendClientMessage(playerid, 0xff8400ff, "Create entrance: /centrance [entrance name]");
...
CMD:createexit(playerid, params[])
{
    ...
    format(exit_export, sizeof exit_export, "(%s) %s: CreateDynamicPickup(19198, 6, %0.f, %0.f, %0.f, 0, %d);\r\n", pname, entrance_name, p_exit[0], p_exit[1], p_exit[2], GetPlayerInterior(playerid));
