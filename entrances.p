...
static entrance_export[220], exit_export[220], entrance_name[32], Float:p_entrance[3], last_entrance;
CMD:createentrance(playerid, params[])
{
    ...
    static entrance_mapicon, pname[21];
	if(sscanf(params, "ds[32]", entrance_mapicon, entrance_name))       return SendClientMessage(playerid, 0xff8400ff, "Create entrance: /createentrance [map icon: {E0E0E0}-1 for no mapicon{ff8400}] [entrance name]");
    ...
    SendClientMessage(playerid, 0xAFAFAFAF, "[ENTRANCES] {E0E0E0}Use SA-styled yellow arrow to enter interior, and create exit from building!");
...
CMD:createexit(playerid, params[])
{
    ...
    SetPlayerPos(playerid, p_entrance[0], p_entrance[1], p_entrance[2]);
    return 1;
}